...
export function Runner() {
  ...
  return (
    <div style={{ display: 'flex', height: 'calc(100vh - 52px)', overflow: 'hidden' }}>
      <div style={{ width: '280px', minWidth: '280px', borderRight: '1px solid var(--forge-border)', overflow: 'auto', padding: '16px' }}>
        <Label>prompt</Label>
        <select
          value={selected || ''}
        ...
        {selected && (
          <>
            ...
            <div style={{ marginTop: '12px' }}>
              <Label>user input / context</Label>
              <textarea
                value={userInput}
                onChange={e => setUserInput(e.target.value)}
                placeholder="Board pack context, company name, etc…"
        ...
        {output && !streaming && (
          <div style={{ marginTop: '20px', borderTop: '1px solid var(--forge-border)', paddingTop: '16px' }}>
            ...
            <textarea
              ...
              style={{ ...textareaStyle, height: '60px', marginBottom: '8px' }}
            />
            <button
              onClick={handleSaveRating}
              disabled={rating === null || saved}
              style={{
                width: '100%', background: saved ? 'var(--forge-green)' : 'var(--forge-surface)',
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ borderBottom: '1px solid var(--forge-border)', padding: '8px 16px', background: 'var(--forge-surface)', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          {output && <button onClick={() => setOutput('')} style={{ marginLeft: 'auto', background: 'none', border: '1px solid var(--forge-border)', color: 'var(--forge-muted)', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.65rem', padding: '2px 8px', cursor: 'pointer', borderRadius: '3px' }}>clear</button>}
        </div>
        <div ref={outputRef} style={{ flex: 1, overflow: 'auto', padding: '20px 24px' }}>
          {output ? (
            <pre style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.85rem', lineHeight: 1.8, color: 'var(--forge-text)', whiteSpace: 'pre-wrap', wordBreak: 'break-word', margin: 0 }}>{output}</pre>
          ) : (
...
const textareaStyle = {
  width: '100%', background: 'var(--forge-surface)', border: '1px solid var(--forge-border)',
  color: 'var(--forge-text)', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.7rem',
  padding: '6px 8px', borderRadius: '4px', outline: 'none', resize: 'vertical',
  lineHeight: 1.6,
}
